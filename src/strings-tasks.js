...
function formatTime(minutes, seconds) {
  return `0${minutes}:${seconds < 10 ? seconds.toString().padStart(2, '0') : seconds}`;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < word.length; i += 1) {
    revesed += `${word[i].split('').reverse().join('')}`;
...
function extractNameFromTemplate(value) {
  const sp = value.replaceAll('!', '').split(' ');
  const res = `${sp[1]} ${sp[2]}`;
  return res;
}
...
function getCardId(value) {
  ...
  return suits.indexOf(suit) * 13 + values.indexOf(num);
}
